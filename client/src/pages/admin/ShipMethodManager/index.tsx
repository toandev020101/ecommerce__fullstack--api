...
const ShipMethodManager: React.FC = () => {
  const dispatch = useAppDispatch();
  const navigate = useNavigate();

  const theme: Theme = useTheme();
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [rows, setRows] = useState<ShipMethod[]>([]);
  ...
  const form = useForm<ShipMethodInput>({
    defaultValues: {
      name: '',
      price: 0,
    },
    resolver: yupResolver(shipMethodSchema),
  });
  ...
  useEffect(() => {
    ...
    const getPaginationShipMethod = async () => {
      try {
        const res = await shipMethodApi.getPagination({
          _limit: rowsPerPage,
          _page: page,
          _sort: sort,
          _order: order,
  ...
  const handleOpenEditDialog = (e: any, index: number) => {
    ...
    form.setValue('price', rows[index].price);
  };
  ...
  const handleAddOrEditSubmit = async (values: ShipMethodInput) => {
    try {
      let res: BaseResponse;
      if (openAddOrEditIndexDialog !== -1) {
        res = await shipMethodApi.updateOne(rows[openAddOrEditIndexDialog].id, values);
      } else {
        res = await shipMethodApi.addOne(values);
      }
      ...
      dispatch(
        showToast({
          ...
          type: 'success',
          message: res.message,
          options: { theme: 'colored', toastId: 'shipMethodId' },
        }),
      );
      setOpenAddOrEditIndexDialog(-2);
      setReload(!reload);
  ...
  const handleDeleteRow = async () => {
    try {
      let res: BaseResponse;
      if (deleteRowIndex === -1) {
        res = await shipMethodApi.removeAny(selectedArr);
      } else {
        res = await shipMethodApi.removeOne(rows[deleteRowIndex].id);
      }
  ...
  return (
    <>
      <TitlePage title="Danh sách phương thức giao hàng" />
      <ToastNotify name="shipMethod" />
      ...
      >
        ...
        <Box display="flex" justifyContent="space-between" marginBottom="20px">
          ...
          <Box display="flex" alignItems="center" gap="20px">
            <TextField
              id="outlined-basic"
              label="Tìm kiếm phương thức giao hàng"
              variant="outlined"
              size="small"
              sx={{ width: '280px' }}
              InputProps={{
                      ...
                      <TableRow key={`table-${index}`}>
                        ...
                        <TableCell>
                          <Skeleton animation="wave" width="100%">
                            <Typography>Tên phương thức</Typography>
                          </Skeleton>
                        </TableCell>
                        <TableCell>
                          <Skeleton animation="wave" width="100%">
                  ...
                  {rows.map((row, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell sx={{ fontSize: '14px', color: theme.palette.error.main, fontWeight: 500 }}>
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description" sx={{ fontSize: '16px' }}>
              ...
              {deleteRowIndex === -1
                ? selectedArr.length + ' phương thức giao hàng này'
                : `phương thức giao hàng "${rows[deleteRowIndex]?.name}"`}{' '}
              hay không?
            </DialogContentText>
          </DialogContent>
          <DialogActions>
      ...
      >
        <DialogTitle id="alert-dialog-title" sx={{ fontSize: '18px' }}>
          {openAddOrEditIndexDialog === -1 ? 'Thêm mới' : 'Chỉnh sửa'} phương thức giao hàng
        </DialogTitle>
...
export default ShipMethodManager;
