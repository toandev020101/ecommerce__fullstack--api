...
import variationOptionSchema from '../../../../validations/variationOptionSchema';

const VariationOptionProduct: React.FC = () => {
  ...
  const [rows, setRows] = useState<VariationOption[]>([]);
  ...
  const form = useForm<VariationOptionInput>({
    defaultValues: {
      value: '',
      slug: '',
      variationId: parseInt(variationId as string),
    },
    resolver: yupResolver(variationOptionSchema),
  });

  useEffect(() => {
    const getOneVariationById = async () => {
      try {
        const res = await variationApi.getOneById(parseInt(variationId as string));
        setVariation(res.data as Variation);
      } catch (error: any) {
  ...
  useEffect(() => {
    ...
    const getPaginationVariationOption = async () => {
      try {
        const res = await variationOptionApi.getPaginationByVariationId(parseInt(variationId as string), {
          _limit: rowsPerPage,
          _page: page,
          _sort: sort,
          _order: order,
    ...
    getPaginationVariationOption();
  }, [dispatch, navigate, order, page, rowsPerPage, sort, reload, searchTerm, variationId]);
  ...
  const handleOpenEditDialog = (e: any, index: number) => {
    ...
    form.setValue('value', rows[index].value);
  ...
  const handleAddOrEditSubmit = async (values: VariationOptionInput) => {
    try {
      let res: BaseResponse;
      if (openAddOrEditIndexDialog !== -1) {
        res = await variationOptionApi.updateOne(rows[openAddOrEditIndexDialog].id, values);
      } else {
        res = await variationOptionApi.addOne(values);
      }

      dispatch(
        showToast({
          page: 'variationOptionProduct',
          type: 'success',
          message: res.message,
          options: { theme: 'colored', toastId: 'variationOptionProductId' },
        }),
      );
      setOpenAddOrEditIndexDialog(-2);
      setReload(!reload);
  ...
  const handleDeleteRow = async () => {
    try {
      let res: BaseResponse;
      if (deleteRowIndex === -1) {
        res = await variationOptionApi.removeAny(selectedArr);
      } else {
        res = await variationOptionApi.removeOne(rows[deleteRowIndex].id);
      }
  ...
  return (
    <>
      ...
      <ToastNotify name="variationOptionProduct" />
      <Breadcrumbs aria-label="breadcrumb" sx={{ marginBottom: '20px' }}>
        <Link to="/quan-tri/san-pham/thuoc-tinh">Danh sách thuộc tính</Link>
        <Typography color="text.primary">{variation?.name}</Typography>
      </Breadcrumbs>
      ...
      >
                  ...
                  {rows.map((row, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell sx={{ fontSize: '14px' }}>{row.value}</TableCell>
        ...
        >
          <DialogTitle id="alert-dialog-title" sx={{ fontSize: '18px' }}>
            Xác nhận xóa chủng loại
          </DialogTitle>
          <DialogContent>
            <DialogContentText id="alert-dialog-description" sx={{ fontSize: '16px' }}>
              Bạn chắc chắn muốn xóa{' '}
              {deleteRowIndex === -1
                ? selectedArr.length + ' chủng loại này'
                : `chủng loại "${rows[deleteRowIndex]?.value}"`}{' '}
              hay không?
            </DialogContentText>
          </DialogContent>
          <DialogActions>
      ...
      >
        <DialogTitle id="alert-dialog-title" sx={{ fontSize: '18px' }}>
          {openAddOrEditIndexDialog === -1 ? 'Thêm mới' : 'Chỉnh sửa'} chủng loại
        </DialogTitle>
...
export default VariationOptionProduct;
