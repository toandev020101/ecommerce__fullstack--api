...
const PaymentMethodManager: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const getPaginationPaymentMethod = async () => {
      try {
        const res = await paymentMethodApi.getPagination({
          _limit: rowsPerPage,
          _page: page,
          _sort: sort,
          _order: order,
  ...
  const handleOpenEditDialog = (e: any, index: number) => {
    ...
    form.setValue('description', rows[index].description);
  };
  ...
  const handleAddOrEditSubmit = async (values: PaymentMethodInput) => {
    try {
      let res: BaseResponse;
      if (openAddOrEditIndexDialog !== -1) {
        res = await paymentMethodApi.updateOne(rows[openAddOrEditIndexDialog].id, values);
      } else {
        res = await paymentMethodApi.addOne(values);
      }
      ...
      dispatch(
        showToast({
          ...
          type: 'success',
          message: res.message,
          options: { theme: 'colored', toastId: 'paymentMethodId' },
        }),
      );
      setOpenAddOrEditIndexDialog(-2);
      setReload(!reload);
  ...
  const handleDeleteRow = async () => {
    try {
      let res: BaseResponse;
      if (deleteRowIndex === -1) {
        res = await paymentMethodApi.removeAny(selectedArr);
      } else {
        res = await paymentMethodApi.removeOne(rows[deleteRowIndex].id);
      }
  ...
  return (
    <>
      <TitlePage title="Danh sách phương thức thanh toán" />
      <ToastNotify name="paymentMethod" />
      ...
      >
        ...
        <Box display="flex" justifyContent="space-between" marginBottom="20px">
          ...
          <Box display="flex" alignItems="center" gap="20px">
            <TextField
              id="outlined-basic"
              label="Tìm kiếm phương thức thanh toán"
              variant="outlined"
              size="small"
              sx={{ width: '280px' }}
              InputProps={{
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description" sx={{ fontSize: '16px' }}>
              ...
              {deleteRowIndex === -1
                ? selectedArr.length + ' phương thức thanh toán này'
                : `phương thức thanh toán "${rows[deleteRowIndex]?.name}"`}{' '}
              hay không?
            </DialogContentText>
          </DialogContent>
          <DialogActions>
      ...
      >
        <DialogTitle id="alert-dialog-title" sx={{ fontSize: '18px' }}>
          {openAddOrEditIndexDialog === -1 ? 'Thêm mới' : 'Chỉnh sửa'} phương thức thanh toán
        </DialogTitle>
